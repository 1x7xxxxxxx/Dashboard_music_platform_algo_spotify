...
- first_seen: 2026-09-12
- History:
  - 2026-09-16: rejouée DEUX FOIS le même soir, sous une forme nouvelle : la suite n'a pas rendu de faux verdicts, elle est **morte en route** — une fois à 11 %, une fois sans rien écrire. J'éditais le Dockerfile, un module de démarrage et trois fichiers de test pendant qu'elle tournait.
    ⚠️ Seconde leçon, sur la SORTIE : les deux passaient par `| tail -6`, qui bufferise jusqu'à la fin du tube. Mortes, elles n'ont laissé **aucune ligne** — ni progression, ni erreur. Une suite longue écrit dans un FICHIER (`> f` ou `| tee f`) ; sinon son agonie est muette et on ne sait même pas où elle en était.
