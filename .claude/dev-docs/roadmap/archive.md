...
- [x] **R85 — Cacher les lectures de `platform_timeseries`** (P3) — LIVRÉ le 2026-09-11.
  Mesuré d'abord : le SQL pèse **103 ms sur un rendu de 287** en production, et **88 %**
  vient de ce seul module. Une revue `code-critic` a REFUSÉ la première conception sur un
  point bloquant : ces fonctions sont écrites pour *ne jamais lever* et rendent vide sur
  panne de base — les envelopper aurait mis la PANNE en cache, transformant une coupure
  d'une seconde en « aucune donnée » pendant 600 s **pour tous les spectateurs**. Le cache
  est donc placé **à l'intérieur** de l'avalement, au niveau de la requête
  (`platform_timeseries.set_fetch()` + `utils/series_cache.py`) : une lecture qui lève
  n'est jamais mémorisée, le module dégrade comme avant, le rendu suivant réessaie. La clé
  `(sql, params)` porte toujours `artist_id`, donc l'isolation entre locataires est
  **structurelle** et non une convention à retenir. Purge branchée sur `clear_kpi_caches()`,
  lui-même appelé aux cinq gestes qui changent la donnée en pleine journée. Effet mesuré :
  l'accueil artiste passe de **13 à 11** requêtes SQL. Garde
  `tests/test_a_failed_read_is_never_cached.py`, mutation vue rouge en reproduisant
  exactement la conception refusée. ref: DEVLOG#2026-09-11.
