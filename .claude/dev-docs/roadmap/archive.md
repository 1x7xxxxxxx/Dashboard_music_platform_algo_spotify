...
et les consignes permanentes sont restées dans l'actif.

### Sous l'index des tâches ouvertes
...
### Sous la table « En attente de toi »
...
chacune commitée et poussée séparément — `33b9c51`, `dced3bd`, `7d6c208`, `946077f`,
`6d35833`, `d5242cd`. Le corps ci-dessous est le bloc de l'actif, rapatrié verbatim :
il est écrit au présent de l'ouverture, et le garder tel quel est ce qui permet de
relire la prédiction À CÔTÉ de son résultat.

### Le résultat, mesuré

| grandeur | avant | après |
|---|---:|---:|
| `swept_by_rerunning_the_guard` — faux balayages | 97 | **0** |
| `sites_unknown` — verdict illisible | 100 | **2** |
| sites vivants trouvés | 100 | **410** |
| taux de trouvaille sur verdicts lisibles | 0,129 | **0,263** |

```bash
make error-health
python3 -c "import json; a=json.load(open('.claude/dev-docs/error-class-health.json'))['aggregate']; print(a['holes']['swept_by_rerunning_the_guard'], a['holes']['sites_unknown'], a['sweep_yield'])"
```

**La cible est atteinte, et dépassée sur le second chiffre** : la ligne d'index annonçait
`97 → 0` et `100 → 3`, la mesure rend `0` et `2`.

⚠️ **Les 2 `sites_unknown` restants sont DÉLIBÉRÉS, pas des oublis.** L'un est un motif
qu'on n'a jamais vu mordre ; l'autre porte 13 sites dont trancher demande de relire chaque
test **avec sa surface vidée**. Les laisser non conclus et le DIRE est le contraire de les
compter comme balayés — c'est précisément le geste que R137 corrigeait.

⚠️ **Prédiction du 1er commit RÉFUTÉE**, et conservée ici parce que c'est la partie la
plus utile de l'entrée : elle annonçait ≈9 classes trouveuses et ≈23,5 sites pour 70
balayages ; mesuré **41 et 182**. **Le facteur ne se cite PAS comme un rendement** —
0,129 portait sur TOUS les balayages, alors que les 97 étaient sélectionnées pour
n'avoir *jamais* été balayées. Les comparer est `anchor-a-number-to-its-population`.
Ce qui tient, et qui ne dépend d'aucun ratio : **182 défauts réels dans du code dont
personne ne s'était plaint** — 410 au terme du chantier.
...
- [x] **R137 — balayer les 101 classes vivantes dont le verdict n'est pas lisible, famille par famille, et corriger ce qui sort.**
...
## 🚪 R138 — La porte qui refuse un faux balayage AU MOMENT DE L'ÉCRIRE (livrée 2026-09-18)

**Livrée et poussée le 2026-09-18**, commit `44f37c8`. Elle ferme par une PORTE ce que
R137 a fermé à la main : un `siblings:` en `swept:` qui n'est qu'une relance de garde est
désormais refusé à l'écriture, pas constaté à la régénération suivante.

### Le résultat, mesuré

```bash
python3 .claude/scripts/audit_runner.py --sweep-verdict
# ▶ sweep-verdict: 402 balayage(s) déclaré(s), 2 non concluant(s) assumé(s)
# ✅ tout `swept:` porte un verdict lisible          → exit 0

grep -n "sweep-verdict\|--coverage\|audit_unreachable" .github/workflows/ci.yml
# 214: audit_runner.py --sweep-verdict
# 220: audit_runner.py --coverage
# 221: audit_unreachable_tools.py

.venv/bin/python -m pytest tests/test_the_sweep_gate_is_wired_where_it_blocks.py -q
```

⚠️ **La correction de prémisse est GARDÉE, c'est la leçon de la brique.** La ligne d'index
annonçait trois contrôles « qui ne tournent qu'à la main », dont `check_config_refs.py` —
**déjà en CI depuis plus longtemps**. Mesuré en lisant `ci.yml` avant de câbler :
**deux manquaient, pas trois** (`audit_unreachable_tools.py` et `--coverage`). Les deux
sont câblés, plus la porte `--sweep-verdict`. Le trou réel était plus petit et mieux nommé
que celui qu'on s'était annoncé — et c'est la seconde fois dans cette même brique qu'une
affirmation sur la CI tombe en la LISANT (voir le ⚠️ sur `--prose` ci-dessous).
...
- [x] **R138 — `audit_runner.py --sweep-verdict`, et `config-check` câblé en CI.**
...
## 🧩 R118 — `st.fragment` sur les vues à filtres, 6/11 puis clos sur mesure (close 2026-09-17)

- [x] **R118 — `st.fragment` sur les vues à filtres. AVANCÉE 6/11, et sa population est
      à redériver.**
...
## 📦 Rotation du 2026-09-03 — décisions, pas tâches ouvertes

Ces blocs portaient les **14 derniers `- [ ]`** du fichier actif. Aucun n'est un travail
qu'on peut commencer : chacun est soit une **décision de ne pas faire, assortie de son
déclencheur** (ADR-007 : le travail de performance est conditionné, et les quatre
déclencheurs ont été lus contre la production le 2026-08-30 — aucun n'est tiré), soit
un item **bloqué par l'accumulation de temps** (des paires étiquetées qui ne peuvent
pas être saisies d'avance), soit le geste humain déjà porté par **R1** dans la table
« En attente de toi ».

Les laisser cochables entretenait la confusion entre « à faire » et « décidé ». Trois
d'entre eux étaient en plus **dupliqués** entre le bloc P4 et le bloc « Base
d'optimisation différée » : caching, migration `view_session()`, god-functions.

Déplacement, jamais suppression — `tests/test_roadmap_two_files.py` échoue si la somme
des deux fichiers rétrécit. Ce qui rouvre chacun est écrit dans le bloc lui-même.

**P4 — tech-debt / opportunités (basse urgence) :**
- [x] **[CLOS — décision, non livré]** **Caching** — 4 vues requêtent la DB sans `@st.cache_data` (`spotify_s4a_combined`, `meta_ads_overview`, `export_pdf/csv`, `usage_analytics`). Bénéfice **modeste** à l'échelle actuelle (requêtes <1ms mesurées) ; vrai levier LCP = cache Cloudflare (en cours). Effort M.
- [x] **[CLOS — décision, non livré]** **`view_session()` migration** — 16 vues encore en `get_db_connection()` legacy (valide mais non-conforme rule #9). Tech-debt, **pas un leak**. Effort M.
- [x] **[CLOS — décision, non livré]** **171 fonctions >40 lignes** (règle projet) — surtout des `show()` Streamlit (jusqu'à 502 l. `meta_ads_overview`). Lisibilité. Effort L. (cf. `refactor-audit-dashboard.md`)
...
- [x] **[CLOS — décision, non livré]** **Caching `@st.cache_data(ttl=300)` sur les 4 vues lourdes** (`spotify_s4a_combined`, `meta_ads_overview`, `export_pdf/csv`, `usage_analytics`). *Gain* : évite la re-requête à chaque rerun Streamlit. *Risque* : cacher la donnée pure (pas `db`/connexion → unhashable), staleness TTL. *Déclencheur* : trafic concurrent / re-renders fréquents ressentis. Effort M.
- [x] **[CLOS — décision, non livré]** **Migration `view_session()` (16 vues legacy `get_db_connection()`)** — classe `view-session-adoption`. *Gain* : robustesse connexions (graphify : `get_db_connection` = 57 edges). *Risque* : refactor mécanique 16 fichiers = régression. *Déclencheur* : ≥50 artistes / si un leak de connexion apparaît. Effort M.
- [x] **[CLOS — décision, non livré]** **Splitter les god-functions** (`collect_report_data()` = 69 edges, + 171 fonctions >40 l. règle projet). *Gain* : lisibilité/maintenabilité, **pas perf**. *Risque* : élevé si fait en masse. *Déclencheur* : **au fil de l'eau** quand on touche déjà le fichier (jamais en sweep dédié). Effort L.
- [x] **[CLOS — décision, non livré]** **Lazy imports** (plotly/sklearn/shap en tête de vue → différer dans les fonctions). *Gain* : cold-start par vue. *Risque* : faible mais large. *Déclencheur* : si latence par-vue ressentie. Effort M.
- [x] **[CLOS — décision, non livré]** **Index composite `s4a_song_timeline(artist_id, song, date)`** — **prématuré aujourd'hui** (mesuré 0.4ms / 13794 lignes). *Déclencheur* : **~10× le volume de données** (≈140k lignes) ou EXPLAIN qui régresse. Effort S.
...
- [x] **[CLOS — décision, non livré]** **Automated retraining on live outcomes** — `data_anon.csv` is a one-time snapshot. **Blocker = forward outcomes accruing in time:** needs `ml_song_predictions` to gather real trigger results (score → submit to playlists → observe DW/RR/Radio weeks later).
  ...
  - [x] **[CLOS — décision, non livré]** **Champion/challenger retraining DAG** — consume accumulated `ml_prediction_outcomes` pairs to retrain + compare vs the live model. Still genuinely blocked: needs enough labelled cycles to have accumulated (forward time + entries). Build once `ml_prediction_outcomes` has a meaningful row count.
...
- [x] **[CLOS — décision, non livré]** **Resurrection tuning** — thresholds in `detect_saves_resurrection` (min_age 180d, 2x baseline, min_spark 50) are heuristic; recalibrate once a real **saves time-series** exists (an old song's saves spiking months later) — a longitudinal history, not a snapshot.
...
- [x] **[CLOS — décision, non livré]** **E1 — Beta privée avec des proches** (P3, AVANT tout marketing) — `streamlytics.fr` déployé mais
...
- [x] **[CLOS — décision, non livré]** **E2 — Landing page marketing + pixel + CAPI** (P3 growth, APRÈS E1) — promouvoir l'app via
  ...
  - [x] **[CLOS — décision, non livré]** **Landing statique SÉPARÉE de l'app** : `streamlytics.fr` (racine + `www`) → landing **statique**
  ...
  - [x] **[CLOS — décision, non livré]** **Pixel client sur la LANDING uniquement** : Meta Pixel + GA4 `gtag` + (option) TikTok pixel →
  ...
  - [x] **[CLOS — décision, non livré]** **CAPI server-side depuis FastAPI** (obligatoire ici, pas optionnel) pour les conversions
  ...
  - [x] **[CLOS — décision, non livré]** **Pont d'attribution (stitching)** — GRATUIT grâce aux sous-domaines : le pixel pose `_fbp`/`_fbc`
...
### ✅ R59-R62 — ouvertes et closes le 2026-09-05
...
- [x] **R59 — deux tests encodent des règles opposées sur la même sonde.** P3.
...
- [x] **R60 — `_claimed_count` fabrique le défaut qu'il devait éviter.** P2.
...
- [x] **R61 — la suite de tests sème des lignes fabriquées dans la base de dev.** P2.
...
- [x] **R62 — l'ajout d'un partenaire Meta reste un geste manuel des deux côtés.** P3.
...
**Comment chacune a été close** (DEVLOG « suite 14 ») :

- **R59** — [CLOS — la prémisse était fausse] `ADR-016`. Les deux tests ne se
  contredisent pas : l'un gouverne le chemin **nocturne** (budget, on ne sonde que le
  rouge), l'autre le chemin **interactif** (un humain attend). Deux économies, deux
  règles compatibles. Ce qui manquait était l'écrit, pas un arbitrage.
- **R60** — [CLOS — corrigé] `_claimed_count` rend `None` (« je ne sais pas ») au lieu
  de `0`, et journalise. Le message à deux cas couvre déjà l'inconnu : rendre `ok=True`
  aurait été pire, car sur le chemin nocturne `None` est le cas **courant**.
- **R61** — [CLOS — corrigé] frontière de données dans `tests/conftest.py`, troisième
  du genre après SMTP et HTTP. Mesuré : **9** lignes fabriquées (et non 358) suffisaient
  à faire passer au vert une plateforme dont la dernière vraie collecte datait de
  **85 jours**.
- **R62** — [CLOS — porte fermée, mesurée] `ADR-017`. Le jeton porte
