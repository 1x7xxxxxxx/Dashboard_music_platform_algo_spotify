...
- [ ] **N+1 Airflow DAG monitoring** (HIGH) — `airflow_kpi.py:209`, `home.py:350`, `credentials/_core.py` (`_fetch_dag_last_states`, moved from `credentials.py:118` in the 2026-05-15 package split) : pattern `for dag_id in dag_list: monitor.get_runs_for_dag(dag_id, limit=1)` → ~15 appels API par render. Fix : batch via `monitor.get_kpis()` ou cache 60s. **Gain ~2-3 s/render.**
...
- [ ] **`DaysSinceRelease` uses backfilled timeline MIN(date)** — one-shot CSV backfill makes every song's first-appearance identical, so the age proxy is unreliable; re-derive from `track_release_reference` (per-song match) like ReleaseConsistency. Stored `ml_song_predictions` also keep stale `features_json` until the next `ml_scoring_daily` re-score. (Surfaced 2026-05-29.)
