...
| R37 | `make` absent du serveur de production | P3 | constaté le 2026-08-21 pendant le déploiement : `make migrate` (la séquence que R25 prescrivait) sort en 127 sur la prod. `make deploy` marche parce qu'il fait `ssh … bash tools/deploy.sh`. Soit installer `make` sur l'hôte, soit donner à `migrate` un `tools/migrate.sh` comme `deploy` a le sien — la seconde est cohérente avec ce qui existe. Tant que ce n'est pas fait, toute procédure écrite « lance `make X` sur la prod » est fausse. |
