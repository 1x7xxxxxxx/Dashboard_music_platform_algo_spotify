...
| R21 | ~~Nettoyer les lignes contaminées en PROD~~ | P1 | ✅ **FAIT 2026-08-20** : sauvegarde prise, **5304 lignes** supprimées (GRiNCH 67 vidéos + 603 stats + 1 chaîne ; Cuzebo 4556 stats + 68 historiques — toutes la chaîne de l'admin). `track_popularity_history` = 1051 lignes sous artist_id=1 mais **légitimes** (seul l'admin a des tracks Spotify) — mécanisme armé, dégâts nuls. Vérifié : collecte réelle relancée, admin = ses 67 vidéos, locataires 11/13 = rien. |
| R22 | ~~Appliquer la migration `064` en prod~~ | P1 | ✅ **FAIT 2026-08-20** (additive : index composites + `is_canary`). |
| R25 | Appliquer la migration `065` **avec** le déploiement du code | P1 | ⚠️ **NE PAS appliquer seule** : essayée le 2026-08-20, elle a cassé la collecte YouTube en minutes (le code déployé fait encore `ON CONFLICT (channel_id)`) → revert immédiat, collecte rétablie. Classe `migration-ahead-of-its-code`. Séquence : déployer le code, **puis** `make migrate`. |
| R26 | Déployer le code corrigé en prod | P1 | la prod tourne encore l'ancien code. La fuite est neutralisée **par la configuration** (identités admin retirées de `.env` + `docker-compose.yml`, admin déclaré comme locataire), mais les gardes du code ne sont pas là. Séquence : merge → `make deploy` → migration 065 → `make artist-preflight`. |
| R27 | ~~Alerting muet dans le scheduler~~ | P1 | ✅ **FAIT 2026-08-20** : `SMTP_*`/`ALERT_EMAIL` n'étaient déclarés que pour le service `dashboard` → **672 échecs de watchers CSV en 7 jours, aucun mail**. Câblés dans `airflow-common-env` (prod + exemple), scheduler redémarré, vérifié. Garde : `test_env_contract` élargi aux lectures **transitives** (`src/utils`), qui était son angle mort. |
| R28 | ~~Watchers CSV en échec permanent~~ | P1 | ✅ **FAIT 2026-08-20** : `PermissionError` sur `/opt/airflow/data/raw` (volume `root:root`, airflow en uid 50000). 672 runs échoués depuis le 13/08. Droits corrigés, run manuel **et** run planifié verts. |
| R23 | ~~Retirer les `DEFAULT 1` sur `artist_id`~~ | P2 | ✅ **ÉCRIT + VÉRIFIÉ** : `migrations/068`. DEFAULT retiré (55 colonnes) **et** `NOT NULL` posé (81) — l'oubli du locataire devient fatal. 805 tests verts contre une base la portant. ⚠️ **attend le déploiement du code** (bannière d'ordre, comme 065). |
| R29 | ~~`make schema-check` aveugle aux contraintes~~ | P2 | ✅ **FAIT** : comparaison étendue aux PK/UNIQUE/FK et index uniques, **par définition**. A trouvé 3 dérives inconnues → migrations `066` (2 uniques aveugles au locataire, supprimées en prod) et `067` (3 FK Meta manquantes, ajoutées en prod). |
| R30 | ~~Deux locataires, même identifiant plateforme~~ | P2 | ✅ **FAIT** : `find_identity_conflict()` refuse à l'enregistrement sur les 4 plateformes ; test garantissant qu'aucune plateforme ne peut être ajoutée sans règle d'unicité. |
| R31 | ~~« Lancé ! » sans résultat~~ | P3 | ✅ **FAIT** (= item B2 de R14) : `collection_progress.py` garde le `run_id`, lit l'état à chaque rerun et traduit l'échec en geste. Échec non reconnu ⇒ pas d'explication inventée. |
| R32 | ~~Parcours d'inscription non testé~~ | P2 | ✅ **FAIT** : `tests/test_signup_funnel_db.py` — paire user/tenant atomique, non vérifié + jeton, mot de passe haché, slug unique, locataire frais cohérent (aucune ligne, readiness « à connecter »). |
...
| R34 | Trancher le sort de `.claude/dev-docs/architecture/` | P3 | **décision, pas défaut** : ce dossier est une SECONDE surface d'architecture, issue du payload baseline. `generate-dev-docs.py` remplit `api/endpoints.md` (8 routes) et `database_schema.md` (47 tables, 536 TODO de colonnes) mais **pas** `macro_architecture.md` (ses extracteurs trouvent 0 module ici). Le doc réel et peuplé est `.claude/dev-docs/architecture.md`. Soit on peuple, soit on retire — mais deux surfaces dont une vide, c'est ce qui rendait `/review-architecture` inutilisable. |
| R35 | Conformité baseline : **76,2/100** — combler les 3 axes faibles | P3 | mesuré par `audit_fleet.py --project .`. **Axe F contexte 4/10** : CLAUDE.md fait 427 lignes (>400 = −3) — les tableaux descriptifs d'agents/skills/commandes y coûtent du contexte à chaque session alors que le dépôt a mesuré qu'un agent cité dans un tableau n'est **jamais** invoqué (0/23). Les déplacer en doc de référence rendrait ~3 points sans perdre une règle. **Axe E 6,7/10** : `code-architecture-reviewer` et `web-research-specialist` ne sont nommés que dans des tableaux (utilisables à la main, jamais déclenchés). **Axe G 7,1/10** : 5 fichiers modifiés localement vs payload — normal et voulu, à ne pas « corriger » par un re-push. |
| R36 | Fuite de domaine restante dans la config | P3 | ✅ **l'essentiel FAIT le 2026-08-21** (`rules/python.md` réécrite, `/review-architecture` réécrite, `code-critic` recentré, `security-reviewer` retiré). RESTE : vérifier les hooks et scripts qui mentionnent encore QuestDB/MSDR (`inject_context.py`, `sensor.py`, `check_ci_waste.py`, `commands/{adr,dev-docs,check-env,audit-collectors}.md`) — la plupart sont des exemples inoffensifs, mais chacun doit être lu une fois. Signature heuristique : `grep -rliE "opc.?ua|questdb|MSDR" .claude/ --include=*.md --include=*.py`. |
