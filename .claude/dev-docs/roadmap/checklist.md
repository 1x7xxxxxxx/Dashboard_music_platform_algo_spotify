...
| R9 | `view_session()` — **5 vues ouvrent 2 à 5 connexions par rendu** | P3 | requalifié le 2026-08-21 : la fiche disait « 16 vues legacy, valide mais non conforme, tech-debt, **pas un leak** ». La mesure dit autre chose — `admin.py` **5** connexions par rendu, `hypeddit.py` **5**, `airflow_kpi.py` **4**, `export_csv.py` et `export_pdf.py` **2** — ces deux dernières **corrigées le 2026-08-21** : chacune ouvrait un `db2` que la règle #9 interdit nommément, alors que `db` était encore ouverte (et, pour `export_pdf`, déjà passée en paramètre). Elles sont à 1. **`airflow_kpi` aussi (4 → 1)** : ses quatre sections ouvraient chacune la leur, et Streamlit exécute le corps de **chaque onglet** à chaque rerun, donc les quatre partaient à tous les coups — `show()` en ouvre désormais une, la passe aux sections et la ferme une fois. Restent **deux** vues, 5 et 5 — et je m'y suis arrêté sciemment le 2026-08-21. `admin.py` (901 l., 10 `close()` pour 5 ouvertures) porte l'**effacement RGPD** et la **rotation de tokens** ; `hypeddit.py` écrit depuis un **callback de formulaire**. Ce sont des chemins d'écriture derrière des boutons, et le render-smoke ne clique sur rien : il passerait au vert sur un bouton cassé — c'est exactement le trou constaté sur les vues d'export, qu'il a fallu combler par un test qui appuie. Le préalable est donc ce test-là, pas le refactor. Leurs connexions vivent dont les connexions vivent dans des helpers appelés hors du rendu (dont un callback de formulaire) : ce n'est pas mécanique, c'est un changement dédié et relu. La règle #9 ne dit pas « préférer une connexion », elle dit qu'une vue en ouvre exactement une et jamais une seconde en repli. Les 13 autres n'en ouvrent qu'une et ne diffèrent que par l'orthographe du garde : conformité, migrable en une ligne quand on touche la vue. **Et la migration en masse est un piège** : `admin`, `airflow_kpi` et `perf_monitor` ne résolvent **aucun** locataire (surfaces admin transverses) là où `view_session()` en exige un, et `referral` **refuse** les admins alors que `view_session()` leur donne `artist_id = 1` — la migrer par motif leur ouvrirait un programme de parrainage que la vue leur interdit. Plafond tenu par `tests/test_view_connection_budget.py` : un nombre ne peut que baisser. Passé P4 → **P3** : cinq connexions par rendu n'est pas du style. |
...
| R14 | Onboarding UX — périmètre **reconstruit depuis le code** | P3 | l'ancien libellé renvoyait à un plan inexistant (voir la note de session 2026-06-19→20). Ce qui est **prouvé dans le code**, chaque item vérifiable seul : **✅ guides** — les 4 plateformes credentials et les 4 imports CSV ont chacun le leur, il n'en manque aucun (`credential_guides.py`, `csv_guides.py`) ; **✅ B2 polling** = R31, clos ; **✅ D1 chaîne YouTube** — un artiste qui collait `@pseudo` ou l'URL de sa chaîne obtenait « Channel ID introuvable », impasse au dernier pas du réglage et échec réel de Benken. `src/dashboard/utils/youtube_channel.py` reconnaît les 4 formes d'URL, le pseudo et l'id, distingue un id tronqué d'un id faux, et **rapporte** l'identifiant résolu sans jamais le substituer (22 tests) ; **RESTE C1 — Meta multi-comptes** : `_platform_meta.py` lit un seul `account_id`, un locataire qui gère deux comptes publicitaires ne peut pas les déclarer. C'est une brique (schéma + collecte + vues), pas un ajustement — à ouvrir seule. |
| R16 | Filtre inutile à enlever (front) | P4 | **cherché par lecture le 2026-08-21, non identifiable — reste une décision d'écran.** Les 7 vues qui portent un filtre de période ont été relues : `instagram`, `soundcloud`, `youtube` n'ont **que** le filtre de période ; `hypeddit` et `meta_x_spotify` y ajoutent un seul sélecteur d'entité ; `apple_music` et `spotify_s4a_combined` combinent sélecteur de titre + période, ce qui est légitime. La détection de date de sortie dans `apple_music` **ne filtre rien** — elle pré-sélectionne le titre au premier chargement, puis `session_state` prend le relais. Aucun filtre redondant démontrable : retirer au jugé serait une régression pour qui s'en sert. À confirmer devant l'écran, en nommant la vue. |
