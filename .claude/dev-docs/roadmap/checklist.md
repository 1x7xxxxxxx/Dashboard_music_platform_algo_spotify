...
| R33 | Fabrique de connexion unique — **fait dans `credential_loader`**, reste 4 modules | P3 | `credential_loader` portait **4** copies du même DSN (mêmes 5 variables, mêmes mots-clés) : extraites dans `_connect(autocommit=False)` le 2026-08-21, garde `TestSingleConnectionFactory` vérifié 3 rouges avant / 15 verts après. RESTE, **au fil de l'eau** : `stripe_webhook.py`, `circuit_breaker.py`, `dag_run_logger.py` et `postgres_handler.py` construisent chacun leur connexion au lieu de passer par `PostgresHandler`. Sciemment reporté : le webhook Stripe est le chemin de l'argent et mérite son propre changement, relu seul. |
