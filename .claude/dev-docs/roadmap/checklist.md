...
- [ ] **Cohérence env-first des 9 `*_schema.py`** (P3 tech debt, identifié 2026-06-13 par l'analyse d'impact)
  — `apple_music_csv/app_costs/distrokid_csv/distrokid/hypeddit/imusician_csv/imusician/instagram/stripe/wrapped/
  youtube`-schema font `config = config_loader.load(); db_config = config['database']` en **subscript direct sans
  fallback `DATABASE_URL`** → `KeyError` si lancés en prod (pas de config.yaml). **Hors chemin runtime** (les
  tables prod viennent du dump/migrations, pas de ces `create_tables()`), donc impact faible — mais incohérent
  avec le reste (tout le runtime est env-first) et footgun. Fix : réutiliser `get_db_connection()` / `DATABASE_URL`
  d'abord. ref: DEVLOG#2026-06-13 (suite 14, analyse d'impact).
