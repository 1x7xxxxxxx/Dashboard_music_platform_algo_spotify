...
|----|-------|------|--------------------------|
| R13 | **Régénérer le token Meta System User (cassé en prod)** | P2 | **action utilisateur — Meta Business Manager, je ne peux pas m'y connecter.** Token malformé (`EE…`), code-190 sur tout REST ; le SDK survit sur des fenêtres vides, ce qui rend la panne silencieuse côté DAG. Vérifier aussi `META_APP_ID/SECRET`. **Corrigé le 2026-08-21, ce qui n'était pas la panne mais son silence** : `alert_monitor` n'avait que des contrôles **par locataire** et aucun ne demandait si l'app partagée authentifie encore — `check_central_apps.py` existait et n'était planifié nulle part, exactement la classe des 672 échecs muets de watchers. La tâche `check_central_apps` tourne désormais chaque nuit, en **tête** du sujet et du corps de l'e-mail (une app partagée HS met toute la flotte à terre, et les alertes par locataire en dessous en sont souvent la conséquence), et un import impossible se signale au lieu de ressembler à « tout va bien ». Garde : `tests/test_central_apps_are_monitored.py` (4 rouges avant / 5 verts après). |
