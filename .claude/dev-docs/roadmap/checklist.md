...
| R136 | **108 assertions de présence qu'un commentaire peut satisfaire** — mesuré le 2026-09-18 par `python3 .claude/scripts/audit_presence_assertions.py` : 110 sites de la forme `assert "<mot>" in <texte brut d'un fichier>`, dont **2 vérifiés par mutation et corrigés**. Sur l'un d'eux le défaut a été remis en place EN ENTIER (CTE `linked` inlinée, migration 119) et les 16 tests du fichier sont restés VERTS : le mot `MATERIALIZED` vivait aussi dans le commentaire qui l'explique. Classe `guard-satisfied-by-its-own-comment`. ⚠️ Ce n'est pas un défaut applicatif : c'est une couverture qu'on croit avoir. Le tri demande d'ouvrir chaque site (la variable est résolue à l'exécution, le balayage ne peut pas trancher seul) | P3 | le nombre de sites rendu par la signature, et combien ont été relus | 

**Quatre tâches sont ouvertes dans cet index** — R122, R132, R133, R134 — et l'ancre
...
<!-- reprise: open=R122,R125,R132,R133,R134,R135,R136 -->
