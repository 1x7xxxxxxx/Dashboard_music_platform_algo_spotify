...
- [ ] **Meta Ads DAG first-run backfill** — token blocker RESOLVED 2026-05-31; now gated on Meta rate limit (transient). (1) The expired personal token (auth `code 190`) was replaced by a valid **System User token** (verified `type=SYSTEM_USER`, `expires_at=0`, all scopes), `expires_at` set NULL in DB. (2) The backfill run then failed on Meta **BUC rate limit `code 80004`** ("too many calls to this ad-account") — fails on the very first `/campaigns` call. Root cause this session: **5 concurrent manual runs** hammered the same ad-account → instant throttle. All killed, DAG paused to let the quota window reset (~30-60 min). **To finish:** unpause + trigger ONE single run (no concurrency) once the window clears, or let the daily scheduled run (0 5 UTC) pick it up with fresh quota. Confirm: `rows_inserted > 0`, `meta_insights_performance_day` latest day advances past 2024-09-30. NB: this is exactly the open "Meta per-chunk insight persistence" pain — a late throttle discards the whole run.
...
- [ ] **Lazy imports plotly + pandas dans 19 vues** (MEDIUM) — DE-PRIORITIZED 2026-05-31 after re-analysis (was: blocked on no render-coverage — now unblocked by `tests/test_views_render_smoke.py`). Architecture already lazy-loads view modules per page (`app.py` dispatch: `elif page=="x": from views.x import show; show()`), and the module-import + `show()` call are coupled, so deferring `import plotly` into `show()` saves ≈0 — plotly still loads on the first chart render. Cold start is dominated by the 532 KiB JS bundle (checklist: "irréductible sans changer de framework"), which masks any Python-side ms. 26-file churn for no user-visible gain → not worth it. Revisit only if a non-charting view is added.
...
- [ ] **Meta Ads per-chunk insight persistence** — a throttle on a late aggregate call currently discards all already-fetched insights of the run. Persist insights per chunk so a late throttle doesn't lose earlier work. (Surfaced 2026-05-29; not yet scoped.)
...
- [ ] **Meta campaign-grain breakdowns keyed by `campaign_name`** — add a prune/rename-guard. A campaign RENAME could re-introduce stale rows under the old name (ad/adset grains key by ID, immune). Low priority — single-writer consolidation already eliminates the duplication source. (Surfaced 2026-05-29.)
...
- [ ] **RR calibration curve missing** — no `Calibration_Curve_RR.png` artifact exists (only DW has one), so RR ships with NO calibration bands like RADIO. To fill: the ML notebook must emit a per-bin Calibration_Curve_RR.png (predicted prob vs observed frequency), then populate `ALGO_CALIBRATION_BANDS["RR"]`. (Surfaced 2026-05-30.)
- [ ] **Replace provisional `algo_lifecycle_benchmark` seed with real export** — current 18 rows are qualitative P25/median/P75 shapes from user notes (`total_stream_median` NULL). Run `machine_learning/export_lifecycle_benchmark.py` against a real `data_anon.csv` export and re-seed via a new migration. (Surfaced 2026-05-29.)
- [ ] **Phase 2 — live per-algorithm stream capture from S4A** — `s4a_song_timeline` is total streams only (no DW/RR/Radio split), so per-tenant live lifecycle curves are impossible today; the benchmark must stay a static global cohort until a per-algorithm capture path exists. **WAVE 5 dependency (2026-05-30):** the volume layer also blocks here — `NonAlgoStreams28Days` (the DW regressor's #1 SHAP driver), `DiscoveryMode` and `RadioCount` are imputed to 0.0 until this capture path exists, so `ALGO_VOLUME_ZONES` / the organic budget-scaling threshold ship in rule+static-target mode ("cible, pas écart live") and auto-upgrade to live deltas at Phase 2. **WAVE 6 add (2026-05-30):** the Radio superstar lever `HowManySongsDoYouHaveInRadioRightNow` is the same imputed-0 class — its volume bonus zone renders pedagogically (`live_unavailable`) and goes live at Phase 2; the `radio_discovery_recovery_note` cruising trigger fires off live `StreamsLast7Days` (available) so it works today. (Surfaced 2026-05-29.)
