...
| R20 | Canari — ✅ **local fait et vert**, prod : **une ligne à ajouter à la main** | P2 | **Local (2026-08-21)** : `artist_id=471`, préflight vert de bout en bout. Il a immédiatement révélé trois défauts (`identity-mirrored-but-written-once` P1, `api-partial-date-into-date-column` P2, `env-resolved-against-cwd` P2), tous corrigés, gardés et **déployés en prod** (`15f3a19`), registre de migrations posé (71/71). **Prod — le geste : Créer le canari, après avoir ajouté une ligne au compose du serveur.** Pourquoi cette ligne : `tools/` n'est monté dans aucun conteneur alors que psycopg2 n'existe QUE dans les conteneurs, donc la commande du runbook ne s'exécute nulle part (`can't open file '/app/tools/create_canary.py'`). Le montage est ajouté à `docker-compose.example.yml` et gardé par `tests/test_operational_scripts_are_reachable_in_containers.py`, **mais le compose de prod est gitignoré : il n'arrive pas par `git pull`.** Une ligne `- ./tools:/opt/airflow/tools:ro` sous chaque service airflow, puis la commande du runbook §2. ⚠️ Le canari sera collecté chaque nuit — c'est voulu, c'est ce qui le rend détecteur, et ça consomme un peu de quota. |
