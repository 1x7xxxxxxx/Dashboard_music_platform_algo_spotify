...
| R35 | Conformité baseline : **81,1/100** (était 76,2) — deux axes restants | P3 | mesuré par `audit_fleet.py --project .`. **Axe F contexte 4→6/10** : CLAUDE.md est passé de 427 à 371 lignes (sous le seuil de 400) — les tableaux d'agents et de skills sont partis en `dev-docs/tooling-reference.md`, aucune règle perdue. **Axe E 6,7→7,5/10** (6 agents sur 8 atteignables) : `code-architecture-reviewer` a été repointé sur la surface qui existe (R34) mais reste nommé dans un tableau, donc jamais déclenché ; `web-research-specialist` de même. Les rendre atteignables suppose une règle impérative avec un déclencheur vérifiable, pas une ligne de tableau. **Axe G 6,6/10** : fichiers modifiés localement vs payload — normal et voulu, à ne pas « corriger » par un re-push. |
