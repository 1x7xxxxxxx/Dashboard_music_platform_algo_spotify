...
| `admin_accounts.py` | — (sous-module d'Admin, pas une page) — les comptes : utilisateurs, révocation, renvoi de vérification, et l'effacement RGPD Art. 17. Sorti d'`admin.py` le 2026-09-22 quand la page a franchi 1 200 lignes en gagnant son sélecteur à six sections. ⚠️ `_erase_artist_gdpr` est irréversible et voyage avec l'écran qui la déclenche, jamais dans un utilitaire | `saas_users`, `saas_artists`, et les tables purgées | admin |
| `home_meta_advice.py` | — (sous-module de l'accueil, pas une page) — ce que la publicité a appris, en PHRASES et sans une seule jauge : `home_tiles.py` est à 6 `st.metric` pour un plafond de 6. Il ne reçoit ni `db` ni `artist_id`, donc il ne peut pas poser de requête — l'accueil est à son plafond d'allers-retours | aucune : tout vient de `period_side_metrics`, déjà en mémoire | all (le renvoi vers le CPR Optimizer porte le cadenas) |
...
| `usage_analytics.py` | 📈 Usage Analytics — journal `usage_events` de première partie | usage_events | admin |
