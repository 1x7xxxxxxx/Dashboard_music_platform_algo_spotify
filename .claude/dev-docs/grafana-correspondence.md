...
|---|---|---|---|
| `perf_monitor` | Combien de temps met une page à se rendre, et cette machine tient-elle | **Oui** (voir le détail ci-dessous) | **RETIRÉ le 2026-09-16**, la condition ayant été levée |
...
| **Historique session par page** (moy / max / min, tableau) | Panneau 7 « Latence de rendu — PAR PAGE (p95) », ajouté pour cette correspondance | **Mieux, et c'est mesuré.** La vue ne comptait que la phase `view` et ignorait la `chrome`. Le « facteur ~8 » écrit ici le 2026-09-16 au matin était faux : mesuré le soir même côté serveur, la chrome vaut 11-13 ms et la vue 50 à 777 ms. L'omission reste réelle, mais elle était **petite** — voir l'addendum d'ADR-026. |
...
## La condition a été levée — et la suppression est faite

Écrit le matin du 2026-09-16 : « retirer `perf_monitor.py` avant qu'un panneau ait porté
un chiffre reviendrait à remplacer une surface qui marche par une surface vide ». Les
panneaux 1 et 7 étaient alors vides, parce que `streamlytics_rerun_duration_seconds` ne
reçoit d'observation qu'au premier rendu **authentifié** — les routes publiques font
`st.stop()` avant la couture.

**Le soir même, l'utilisateur s'est connecté et a navigué.** Relevé :

```
8 pages : apple_music, data_wrapped, home, instagram, meta_cpr_optimizer,
          meta_mapping, saisie_s4a, soundcloud
phases  : chrome, view     →  CONDITION LEVÉE
```

`perf_monitor.py` et son catalogue i18n sont supprimés, avec la route, l'entrée de
navigation, l'entrée `_ADMIN_ONLY` et la ligne de `architecture.md`.

**Et une deuxième chose est morte avec la vue** : `metrics_seam.record_session_render()`,
qui remplissait `st.session_state["_perf_log"]` — que `perf_monitor` était la SEULE à
lire. Son propre docstring annonçait qu'elle ne survivrait pas à la suppression ; le dire
ne suffit pas à l'enlever. La laisser aurait produit la classe
`du-code-mort-qui-cache-une-consequence-vivante`, que ce dépôt connaît :
`purge_expired()` a vécu des mois dans une `show()` que rien n'atteignait, et plus rien
ne purgeait pendant ce temps.

**Solde de code** : -181 (`perf_monitor.py`) -44 (catalogue i18n) -24
(`record_session_render` + son appel + un import) = **-249 lignes**, pour +3 panneaux
Grafana versionnés.

## Ce que la première mesure a trouvé, et qui n'était pas prévu

La table ci-dessus disait du panneau 7 : « Mieux, et c'est mesuré. La vue ne comptait que
la phase `view` ». C'est vrai, mais la raison écrite le matin — « un facteur ~8 que la
vue n'affichait pas » — était **fausse et dans le mauvais sens**. Mesuré le soir :

| page | chrome | vue |
|---|---|---|
| `meta_mapping` | 12,3 ms | **776,8 ms** |
| `home` | 11,2 ms | 315,9 ms |
| `saisie_s4a` | 10,9 ms | 49,8 ms |
