...
| [PRESET_EXHAUSTIVE_TECHNICAL.md](PRESET_EXHAUSTIVE_TECHNICAL.md) | Frozen preset for long technical reference decks (80–200 slides). Skip Phases 1–2 and use directly when the user asks for an exhaustive deck from a Markdown source — checklists, ML/SoftEng docs, audits. | When source is a `.md` with `<!-- SLIDE_BRIEF -->` block |

## Preset shortcut

If the user says "exhaustive technical deck", "from this checklist", "1000
slides OK", or the source `.md` starts with a `<!-- SLIDE_BRIEF -->` block —
**read [PRESET_EXHAUSTIVE_TECHNICAL.md](PRESET_EXHAUSTIVE_TECHNICAL.md) and
follow it instead of the standard Phase 1/2/3 workflow.** All design
decisions are pre-frozen there (palette, typography, animation policy, slide
density, Mermaid fix, SVG illustration patterns, navigation JS).
