...
EN = {
    ...
    "trigger_algo.algostreams_header": "📈 Streams generated by the algorithmic playlists",
    "trigger_algo.algostreams_caption": "Once a track is picked up by the algos, how many streams each "
                                        "playlist actually generated. Entered in 📝 S4A manual entry. "
                                        "Total = sum of the 3 sources.",
    "trigger_algo.algostreams_no_track": "Pick a track above.",
    "trigger_algo.algostreams_window": "Window",
    "trigger_algo.algostreams_empty": "No data for this track on this window. Enter DW/RR/Radio streams "
                                      "in 📝 S4A manual entry.",
    "trigger_algo.algostreams_total": "Σ Total (cumulative)",
    "trigger_algo.algostreams_y": "Streams",
    "trigger_algo.algostreams_source": "Playlist",
    "trigger_algo.algostreams_table": "📋 Numbers",
    ...
    # ── 🎯 Où en sont mes titres (l'écran d'accueil, 2026-09-22) ─────────────
    "trigger_algo.tab_catalogue": "🎯 Where my tracks stand",
    "trigger_algo.cat.admin": "Pick an artist to see their catalogue.",
    "trigger_algo.cat.unreadable": (
        "Catalogue unreadable right now — that is not the same as « no track »."),
    "trigger_algo.cat.empty": (
        "**Your catalogue has not been scored yet.** The computation runs every night "
        "as soon as a track has at least one stream over the last 35 days. It needs "
        "your Spotify for Artists data: upload it on the **📝 Saisie S4A** page."),
    "trigger_algo.cat.tile_active": "Tracks with a lever",
    "trigger_algo.cat.tile_active_help": (
        "Tracks with at least one measured metric below its target — that is where an "
        "action changes something."),
    "trigger_algo.cat.tile_closest": "Closest to a gate",
    ...
    "trigger_algo.cat.col_missing": "Missing",
    "trigger_algo.cat.col_progress": "Progress",
    "trigger_algo.cat.col_levers": "Levers left",
    "trigger_algo.cat.col_saves": "Saves 28d",
    ...
    "trigger_algo.titre.nothing_left": (
        "✅ No measured metric of this track is below its target. Nothing to fix here "
        "— check the **Where my tracks stand** tab to see which one to push."),
    ...
    "trigger_algo.titre.col_now": "Today",
    "trigger_algo.titre.col_target": "Target",
    "trigger_algo.titre.col_gap": "Missing",
    "trigger_algo.titre.col_progress": "Progress",
    "trigger_algo.titre.col_pays": "What it pays",
    "trigger_algo.titre.col_how": "How",
    "trigger_algo.titre.pareto_note": (
        "⚠️ **This table is sorted by EFFORT, not by impact** — the lever closest to "
        "its target first. Read the « what it pays » column before choosing: measured "
    ...
    "trigger_algo.titre.rate_artist": (
        "your catalogue average — this track has no distributor statement yet"),
    "trigger_algo.titre.money_caveat": (
        "⚠️ **This is not the gain from a trigger.** The reference cohort contains "
        "ONLY tracks that did trigger — no comparable control that did not. It gives "
        "an order of magnitude, not a causal effect. And an expected value is not a "
        "promise: half the draws land below it."),
    "trigger_algo.titre.no_cost": (
        "🚫 **This page says what a gate is WORTH, not what it COSTS to open.** Ad "
        "spend is attached to only 19 tracks and stops in September 2024: a per-track "
        "cost computed from that would be a fiction."),
    "trigger_algo.titre.col_algo": "Algorithm",
    "trigger_algo.titre.col_chance": "Your chance",
    "trigger_algo.titre.col_worth": "Worth if it opens",
    "trigger_algo.titre.col_expect": "Expected",
    "trigger_algo.titre.col_next": "Next lever",
    "trigger_algo.titre.col_cohort": "Cohort",
    "trigger_algo.titre.three_gates_note": (
        "The CLOSEST gate is not the RICHEST one: Discover Weekly is worth about 3.4× "
    ...
    "trigger_algo.titre.best_bet": (
        ...
        "odds, aim for the richer one; at equal value, for the likelier one."),
    # ── 🎛️ Comment régler tes campagnes (2026-09-22) ────────────────────────
    "trigger_algo.reg.header": "🎛️ How to set up your campaigns",
    "trigger_algo.reg.admin": "Pick an artist.",
    "trigger_algo.reg.intro": (
        "Everything you can set on a campaign, ranked by what it ACTUALLY changes on "
        "your account — not by what you read elsewhere. A setting flagged ⚠️ rests on "
        "too few ads to draw a lesson from."),
    "trigger_algo.reg.axis_empty": "No data on this axis yet.",
    "trigger_algo.reg.col_value": "Setting",
    "trigger_algo.reg.col_ads": "Ads",
    "trigger_algo.reg.col_spend": "Spent",
    "trigger_algo.reg.col_cpc": "Cost / click",
    "trigger_algo.reg.col_ctr": "Click rate",
    "trigger_algo.reg.reco": (
        "✅ **Keep « {retenir} »** — {cpc_min:.4f} € per click against {cpc_max:.4f} € "
        "for « {eviter} », i.e. **{facteur:.1f} times cheaper**. Measured on {ads} ads "
        "and {dep} €."),
    "trigger_algo.reg.no_reco": (
        "No recommendation on this axis: it takes **two** settings compared over "
        "enough ads. A single try is not a lesson."),
    "trigger_algo.reg.summary": "📌 **Worth keeping, across all axes**: {gains}",
    "trigger_algo.reg.budget_header": "💰 The budget to trigger each playlist",
    "trigger_algo.reg.budget_nopred": "No prediction for this track yet.",
    "trigger_algo.reg.budget_none": "No gate is waiting on extra streams for this track.",
    "trigger_algo.reg.col_algo": "Playlist",
    "trigger_algo.reg.col_missing": "Streams missing (7 d)",
    "trigger_algo.reg.col_budget": "Order of magnitude",
    "trigger_algo.reg.budget_caveat": (
        "⚠️ **An order of magnitude, not a quote.** The cost per stream is aggregated "
        "over ALL your campaigns and ALL your tracks: it does not say what this one's "
        "streams cost. Per-track attribution would need a campaign↔track mapping, "
        "which exists for only 19 tracks, and ad spend that does not stop in "
        "September 2024."),
    "trigger_algo.reg.roi_header": "⏳ How long until it pays for itself",
    "trigger_algo.reg.roi_unreadable": (
        "Monthly series unreadable — that is not « no data »."),
    "trigger_algo.reg.roi_done": (
        "✅ You are already in the black: net cumulative **{cumul:,.2f} €**."),
    "trigger_algo.reg.roi_date": (
        "At the pace of the last months (**{rythme:,.2f} €/month**), break-even lands "
        "in **{mois} months** — {date}."),
    "trigger_algo.reg.roi_never": (
        "⛔ **At the current pace, never.** The monthly net is zero or negative "
        "(**{rythme:,.2f} €/month** against a **{cumul:,.2f} €** cumulative). Inventing "
        "a distant date would be a polite lie: what needs changing is not patience."),
    "trigger_algo.reg.roi_unknown": "Not enough history yet to say when.",
