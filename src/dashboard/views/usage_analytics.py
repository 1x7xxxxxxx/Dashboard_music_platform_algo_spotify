...
def show():
    ...
    st.caption("Suivi first-party de l'usage de l'app (page_view + actions clés). "
               "Données internes — aucune sortie vers un tiers.")

    if not is_admin():
        st.error("Réservé aux administrateurs.")
        st.stop()
    ...
    if db is None:
        return
    try:
        days = st.selectbox("Fenêtre", [7, 30, 90, 365], index=2,
        ...
        c1.metric("Événements", f"{int(n_events or 0):,}")
        c2.metric("Sessions", f"{int(n_sessions or 0):,}")
        c3.metric("Artistes actifs", f"{int(n_artists or 0):,}")

        if not n_events:
            st.info("Aucun événement sur la fenêtre — clique dans l'app pour en générer.")
            return
        ...
        with col_a:
            st.subheader("📄 Pages les plus vues")
            ...
            if df_pages is not None and not df_pages.empty:
                ...
                st.caption("Les pages absentes ou en bas de liste = candidates « dead feature ».")
        with col_b:
            ...
            if df_evt is not None and not df_evt.empty:
                st.plotly_chart(px.bar(df_evt, x="event", y="n"), width="stretch")

        st.markdown("---")
        st.subheader("👤 Activité par artiste")
