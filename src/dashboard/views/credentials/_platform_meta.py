...
def render_ad_account_picker(artist_id: int, existing_value: str = "") -> str | None:
    """Deux gestes : ouvrir le Gestionnaire, coller l'adresse. Rend le compte trouvé.

    Demandé le 2026-09-04 : « récupérer via l'url après avoir cliqué sur le portail
    business manager pour faciliter le setup à l'artiste : crée une fonction qui ouvre
    l'url et on récupère le n° de act ».

    Ce qu'on NE peut pas faire, et qu'il faut dire ici pour que personne ne le
    retente : lire l'adresse de l'onglet que l'artiste vient d'ouvrir. Un navigateur
    interdit à une page de connaître l'URL d'une autre origine — ce n'est pas une
    limite de Streamlit, c'est la frontière qui empêche n'importe quel site de lire ce
    que vous faites ailleurs. Et l'autre voie — demander la liste des comptes à Meta —
    passe par le jeton de la PLATEFORME : il rendrait les comptes de tous les
    locataires, pas ceux de celui qui regarde.

    Reste donc le geste que l'artiste peut faire et que nous pouvons vérifier : il
    copie sa barre d'adresse, on en extrait `act=` et **on lui montre ce qu'on a
    trouvé avant qu'il enregistre**. C'est cette dernière moitié qui manquait : le
    champ acceptait déjà l'URL entière depuis ce matin, mais rien ne confirmait le
    numéro, donc rien ne distinguait « j'ai collé la bonne page » de « j'ai collé la
    page du Business Manager ».
    """
    import streamlit as st

    from src.utils.tenant_identity import normalise_meta_account

    st.markdown("**" + t("credentials.meta.picker_title",
                         "🔎 Trouver mon numéro de compte publicitaire") + "**")
    st.caption(t(
        "credentials.meta.picker_help",
        # « ① » et non « 1. » : un « 1. » en tête de ligne est une LISTE en markdown,
        # et Streamlit l'a rendue sans son numéro — la consigne commençait donc par
        # « Ouvre… » suivie d'un « 2. » orphelin. Vu au navigateur le 2026-09-04.
        # Les deux boutons ci-dessous portent déjà les mêmes glyphes.
        "① Ouvre le Gestionnaire de publicités et choisis le compte à suivre. "
        "② Copie l'adresse affichée en haut du navigateur et colle-la ici — on en "
        "extrait le numéro."))
    st.link_button(
        t("credentials.meta.picker_open", "① Ouvrir le Gestionnaire de publicités ↗"),
        ADS_MANAGER_URL, use_container_width=False)

    pasted = st.text_input(
        t("credentials.meta.picker_paste", "② Colle ici l'adresse de la page"),
        key=f"_meta_url_paste_{artist_id}",
        placeholder="https://adsmanager.facebook.com/adsmanager/manage/campaigns?act=…",
    )
    if not pasted.strip():
        return None

    found = normalise_meta_account(pasted)
    if found and identity_is_well_formed("meta", found):
        st.success(t(
            "credentials.meta.picker_found",
            "✅ Compte trouvé : **{acct}** — recopie-le dans le champ "
            "**Ad Account ID** ci-dessous, puis **💾 Enregistrer**."
        ).format(acct=found))
        st.code(found, language=None)
        return found

    # Le cas le plus fréquent, et il mérite son propre message : l'adresse du
    # Business Manager, qui porte `business_id=` et ressemble beaucoup à la bonne.
    if "business_id=" in pasted and "act=" not in pasted:
        st.warning(t(
            "credentials.meta.picker_business_id",
            "⚠️ Cette adresse est celle de ton **Business Manager** "
            "(`business_id=…`), pas d'un compte publicitaire. Reviens au "
            "Gestionnaire de publicités et ouvre le compte lui-même : son adresse "
            "contient `act=`."))
        return None
    st.warning(t(
        "credentials.meta.picker_not_found",
        "⚠️ Aucun `act=` dans cette adresse. Vérifie que tu as bien copié la barre "
        "d'adresse **du Gestionnaire de publicités**, une fois un compte ouvert."))
    return None
