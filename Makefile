...
charts-review: ## R204 — verse tes retours dictés (COMMENTS=<fichier yaml>, optionnel), refait le tri par cause (revue/tri.md) et le PDF numéroté, sans base
	@test -f revue/capture.json || { echo "❌ revue/ ne contient pas de capture. Run: make charts-dossier OUT=revue"; exit 1; }
	@.venv/bin/python -c "import weasyprint, yaml" 2>/dev/null || { echo "❌ dépendances absentes. Run: make sync"; exit 1; }
	@$(if $(COMMENTS),.venv/bin/python tools/dev/charts_dossier/apply_comments.py "$(COMMENTS)" revue,true)
	@.venv/bin/python tools/dev/charts_dossier/main.py --rebuild revue
	@.venv/bin/python tools/dev/charts_dossier/triage.py revue
