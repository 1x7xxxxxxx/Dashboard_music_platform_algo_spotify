...
Five roadmap items had sat open for months with the same shape: the engineering
...
These five leave the open roadmap and become **conditions on an input**, each
...
- `.claude/dev-docs/roadmap/archive.md` carries the per-item detail; this ADR
  carries the reasoning that is common to all five.
